...
(function () {
    ...
    function parseCall(expression) {
        ...
        if (!match) return { method: expression, params: [] };
        var method = match[1];
        var raw = (match[2] || "").trim();
        if (!raw) return { method: method, params: [] };
        try {
            return { method: method, params: JSON.parse("[" + raw + "]") };
        } catch (_) {
            // Tolerate single-quoted arguments in the template: 'hello'.
            try {
                return { method: method, params: JSON.parse("[" + raw.replace(/'/g, '"') + "]") };
            } catch (e) {
    ...
    function onClick(event) {
        ...
        if (call) enqueueCall(state, call);
    }
    ...
    function scheduleFlush(state, immediate) {
        ...
        state.debounceTimer = setTimeout(function () { flush(state); }, delay);
    }
    ...
    function flush(state) {
        ...
        fetch(ENDPOINT, {
            ...
            .then(function (payload) {
                ...
                if (state._inFlight !== token) return;
    ...
    function applyServerResponse(state, payload) {
        ...
        payload.components.forEach(function (slice) {
            if (!slice || !slice.snapshot) return;
        ...
        if (activePath) {
            ...
            if (restored && typeof restored.focus === "function") {
                ...
                if (selectionStart !== null && "setSelectionRange" in restored) {
                    try { restored.setSelectionRange(selectionStart, selectionEnd); } catch (_) { /* ignore */ }
                }
            }
        }
    }
    ...
    function morph(existing, incoming) {
        ...
        for (var j = 0; j < existing.attributes.length; j++) {
            ...
            if (name === "wire:id") continue;
            if (!incoming.hasAttribute(name)) toRemove.push(name);
        }
        toRemove.forEach(function (n) { existing.removeAttribute(n); });
    ...
    var Livewire = {
        ...
        call: function (id, method, params) {
            ...
            if (!state) throw new Error("No component with id " + id);
            enqueueCall(state, { method: method, params: params || [] });
        },
